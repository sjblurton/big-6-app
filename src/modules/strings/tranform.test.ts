...
describe("toCapitalizedWords", () => {
  ...
  it("handles a string with multiple spaces between words", () => {
    expect(toCapitalizedWords("  hello   world  ")).toBe("Hello World");
    expect(toCapitalizedWords("  multiple   spaces ")).toBe("Multiple Spaces");
  });
...
describe("workoutIdToTitleString", () => {
  ...
  it("throws an error for invalid workout IDs", () => {
    ...
    invalidIds.forEach((invalidId) => {
      expect(() => workoutIdToTitleString(invalidId as WorkoutIds)).toThrow(
        `Invalid workout id: ${invalidId}`,
      );
    });
  });
